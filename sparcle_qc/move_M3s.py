...
def get_M2_N_resn(bond:str, df:pd.DataFrame, no_HL:Dict[str, str]) -> str:
    """
    ...
    ----------
    bond: str
        number of bond to find resname for
    df: pd.DataFrame
...
def find_M3(bond:str, M2resn:str, df:pd.DataFrame, no_HL:Dict[str,str]) -> str:
    """
    Given a bond number, this function returns an atom number if there is an M3 atom for that bond that is a different resname
    ...
    -------
    M3_to_move: str
        atom number for the M3 atom that has a different residue name than the M2 atom
    """
...
def get_key(val:str, no_HL:Dict[str,str]) -> str:
    """
...
def move_ids(atom_ids: List[str], no_HL:Dict[str,str]) -> Dict[str,str]:
    """
